...
export function initSchedule(targetSelector) {
  ...
  // Ensure Flowbite Datepicker + Supabase exist globally
  if (!window.Datepicker || !window.supabase) {
    console.error("Flowbite Datepicker or Supabase not found — ensure both scripts are loaded.");
    return;
  }

  const container = document.querySelector(targetSelector);
  if (!container) {
    console.error("Target container not found for schedule.js:", targetSelector);
    return;
  }

  /* ---------- Base HTML Injection ---------- */
  container.innerHTML = `
    <div class="card w-full max-w-xl p-6 mx-auto">
      ...
      <div class="flex space-x-2 mb-6 w-full">
        ...
        <button id="tabTour" class="pill pill-off">Tour Schedule</button>
      </div>
      ...
      <div id="panelSingle">
        <div class="mb-6 relative">
          ...
          <input id="srTicketLink" type="text" placeholder="Ticket link (optional) e.g. https://ticketsite.com/event123" class="input-dark p-3 rounded-lg border border-gray-700 text-gray-300 mt-2">
      ...
      <div id="panelTour" class="hidden">
        <div id="tourRoot"></div>
        <button id="addTourBlockBtn" class="mt-6 bg-blue-500 hover:bg-blue-400 text-black font-semibold px-4 py-3 rounded-lg w-full">
  ...
  /* ---------- Styles (mirrored from original <style>) ---------- */
  ...
  style.textContent = `
    ...
    .tour-inline-remove { font-size:18px; line-height:1; width:28px; height:28px; border-radius:9999px; display:flex; align-items:center; justify-content:center;
      background:#0b1220; border:1px solid #374151; color:#cbd5e1; }
    .tour-inline-remove:hover { color:#fff; border-color:#4b5563; }
  `;
  document.head.appendChild(style);

  /* ---------- Supabase + Date Helpers ---------- */
  ...
  const srSearch = container.querySelector('#srVenueSearch');
  const srClear = container.querySelector('#srVenueClear');
  const srResultsPop = container.querySelector('#srResultsPop');
  const srResults = container.querySelector('#srVenueResults');
  const srNoResults = container.querySelector('#srNoVenueResults');
  const srVenueId = container.querySelector('#srVenueId');
  const srNewForm = container.querySelector('#srNewVenueForm');
  const srNewName = container.querySelector('#srNewName');
  const srNewStreet = container.querySelector('#srNewStreet');
  const srNewPC = container.querySelector('#srNewPC');
  const srCreateVenue = container.querySelector('#srCreateVenue');
  let srTimeout;
  ...
  /* ---------- Dates & Repeat ---------- */
  const fromDateEl = container.querySelector('#fromDate');
  const toDateEl = container.querySelector('#toDate');
  const fromTimeEl = container.querySelector('#fromTime');
  const toTimeEl = container.querySelector('#toTime');
  ...
  const repeatTypeEl=container.querySelector('#repeatType');
  const repeatUntilWrap=container.querySelector('#repeatUntilWrap');
  const repeatUntilEl=container.querySelector('#repeatUntil');
  ...
  /* ---------- Tabs & Tour Schedule ---------- */
  const tabSingle=container.querySelector('#tabSingle');
  const tabTour=container.querySelector('#tabTour');
  const panelSingle=container.querySelector('#panelSingle');
  const panelTour=container.querySelector('#panelTour');
  const tourRoot=container.querySelector('#tourRoot');
  const addTourBtn=container.querySelector('#addTourBlockBtn');
